package MailrouteAPI::Customer;

use strict;
use warnings;
use utf8;

=head1 NAME
The Customer Object

=head1 VERSION

Version 0.01

our $VERSION = '0.01';

=head1 SYNOPSIS

=head1 EXPORT

=head1 SUBROUTINES/METHODS
=cut

=head2 schema()

my $cfg = {

	login => 'devrow@gmail.com',

	apikey => '008a4ceb245beb3584493f0269f606dc691dd7f4',

	endpoint => 'https://admin-dev.mailroute.net',

	timeout => 10,
};

my $mr = MailRouteAPI->new(1);

$mr->configure($cfg);

my $customer = $mr->customer();

=cut

=head2 get()

my $r = $customer->get({ id => 12345 });

=cut

=head2 create()

my $r = $customer->create({

	name => 'customer name',

	reseller => $reseller;

});

=over 2

=item * name      required         String - The name of the Customer

=item * reseller  required         String - reseller_uri

=back

=cut

=head2 update()

$rc_customer = $customer->update({

	name => 'name',

	reseller => $rc,

	id => $customer_id

});

=over 2

=item * name      required         String - The name of the Customer

=item * reseller  required         String - reseller_uri

=back

=head2 list()

my $r = $customer->list({

 	order_by => 'field name',

 	limit => 123,

 	offset => 123

});


field name is one of:

=over 2

=item * is_full_user_list

=item * name

=item * branding_info

=item * resource_uri

=item * created_at

=item * allow_branding

=item * updated_at

=item * absolute_url

=item * id

=item * reported_user_count

=item * reseller

=back

=cut

=head2 delete()

my $r = $customer->delete({ id => 12345 });

=cut

my @rest_api = (
	'schema:get:/api/v1/customer/schema/',
	'get:get:/api/v1/customer/',
	'list:get:/api/v1/customer/',
	'create:post:/api/v1/customer/',
	'update:put:/api/v1/customer/{id}/',
	'delete:delete:/api/v1/customer/{id}/',
);

sub new
{
	my ($class, $ua, $debug) = @_;
	my $self = {
		DEBUG => $debug || 0,
		ua => $ua
	};
	bless $self, $class;

	map {
		my ($obj_method, $http_method, $url) = split(/:/, $_);
		my $method_name = 'MailrouteAPI::Customer::' . $obj_method;
		if (!MailrouteAPI::Customer->can($obj_method))
		{
			no strict 'refs';
			*{$method_name} = sub {
				my ($self, $params) = @_;

				my $_url = $url;
				my $_http_method = $http_method;
				my $_obj_method = $obj_method;

				if ($_obj_method eq 'schema')
				{
					;
				}
				elsif ($_obj_method eq 'create')
				{
					$params->{reseller} = $params->{reseller}->{object}->{resource_uri};
				}
				elsif ($_obj_method =~ /update|delete/ig)
				{
					$params->{reseller} = $params->{reseller}->{object}->{resource_uri};
					$_url = $url;
					$_url =~ s/(\{id\})/$params->{id}/i if $url =~ /\{id\}/;
					delete $params->{id};
				}
				elsif($_obj_method eq 'get')
				{
					$_url = $url ."$params->{id}/" if exists($params->{id}) && $params->{id} =~ /[0-9]+/ig;
				}
				elsif($_obj_method eq 'list')
				{
					$params->{reseller} = $params->{reseller}->{object}->{id};
					$_url = "$url?" . join('&', map { $_ . '=' . $params->{$_} } keys %{$params});
				}
				else
				{
					return {
						error_code => undef,
						error_str => 'method not supported',
						object => undef
					};
				}

				die 'url undefined' if !$_url;
				my $object = $self->{ua}->$_http_method($_url, $params);

				return {
					error_code => $self->{ua}->errorCode(),
					error_str => $self->{ua}->errorToString(),
					object => $object
				};
			};
		}
	} @rest_api;
	return $self;
}

=head2 contact()

return MailrouteAPI::CustomerContact object

=cut

sub contact
{
	my ($self) = @_;
	require MailrouteAPI::CustomerContact;
	return MailrouteAPI::CustomerContact->new($self->{ua}, $self->{DEBUG});
}

=head2 contact()

return MailrouteAPI::CustomerAdmin object

=cut

sub admin
{
	my ($self) = @_;
	require MailrouteAPI::CustomerAdmin;
	return MailrouteAPI::CustomerAdmin->new($self->{ua}, $self->{DEBUG});
}

1;
